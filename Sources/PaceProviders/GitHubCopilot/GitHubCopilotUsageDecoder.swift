...
struct GitHubCopilotDecodedUsage: Equatable, Sendable {
    let planName: String?
    let metrics: [GitHubCopilotMetric]
    let isOrganizationManaged: Bool
}

enum GitHubCopilotUsageDecoder {
    private static let monthDuration: TimeInterval = 30 * 24 * 60 * 60

    static func decodeIdentity(_ data: Data) throws -> GitHubCopilotIdentity {
        let response: GitHubIdentityResponse
        do {
            response = try JSONDecoder().decode(GitHubIdentityResponse.self, from: data)
        } catch {
    ...
    static func decodeUsage(_ data: Data) throws -> GitHubCopilotDecodedUsage {
        ...
        guard !metrics.isEmpty || organizationManaged else {
            throw GitHubCopilotProviderError.quotaUnavailable
        }
    ...
    private static func appendSnapshot(
        ...
        guard let metric = snapshotMetric(
            id: id,
            label: label,
            raw: raw,
            resetsAt: resetsAt,
    ...
    private static func resetDate(from root: [String: Any]) throws -> Date? {
        guard let raw = (root["quota_reset_date"] as? String)
            ?? (root["quota_reset_date_utc"] as? String)
...
private struct GitHubIdentityResponse: Decodable {
    let id: Int64
    let login: String
    let name: String?
}
